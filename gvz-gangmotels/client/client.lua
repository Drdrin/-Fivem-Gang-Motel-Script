...
function createBlips()
    for k,v in pairs(Config.Zones) do
			...
			SetBlipColour(blip, v.Pos.colour)
			SetBlipDisplay(206, 2)
...
function getMyMotel()
ESX.TriggerServerCallback('gvz-gangmotels:checkOwnership', function(owned)
    myMotel = owned
end)
end
...
RegisterNetEvent('gvz-gangmotels:cancelRental')
AddEventHandler('gvz-gangmotels:cancelRental', function(room)
    ...
    TriggerServerEvent("gvz-gangmotels:cancelRental", room)
    TriggerEvent("mythic_progressbar:client:progress", {
        name = "renting_motel",
        duration = 2000,
        label = "Otel kontratı iptal ediliyor "..motelRoom,
        useWhileDead = false,
        canCancel = true,
        controlDisables = {
...
function PlayerDressings()
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'room',
	{
		...
		if data.current.value == 'player_dressing' then 
            menu.close()
			ESX.TriggerServerCallback('gvz-gangmotels:getPlayerDressing', function(dressing)
				elements = {}
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'player_dressing',
				{
					...
					TriggerEvent('skinchanger:getSkin', function(skin)
						ESX.TriggerServerCallback('gvz-gangmotels:getPlayerOutfit', function(clothes)
		...
		elseif data.current.value == 'remove_cloth' then
			...
			ESX.TriggerServerCallback('gvz-gangmotels:getPlayerDressing', function(dressing)
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'remove_cloth', {
					...
					menu2.close()
					TriggerServerEvent('gvz-gangmotels:removeOutfit', data2.current.value)
...
AddEventHandler('instance:onEnter', function(instance)
    if instance.type == 'motelroom' then
        ...
        --Citizen.Wait(1000)
               ...
               print('Katılınıyor '..networkChannel)
...
RegisterNetEvent('gvz-gangmotels:enterRoom')
AddEventHandler('gvz-gangmotels:enterRoom', function(room, motel)
    local roomID = nil
    local playerPed = PlayerPedId()
    local roomIdent = room
    local reqmotel = motel
    ESX.TriggerServerCallback('gvz-gangmotels:getMotelRoomID', function(roomno)
    roomID = roomno
    end, room)
    Citizen.Wait(500)
    if roomID ~= nil then
...
RegisterNetEvent('gvz-gangmotels:exitRoom')
AddEventHandler('gvz-gangmotels:exitRoom', function(motel, room)
    local roomID = room
    local playerPed = PlayerPedId()
    Citizen.Wait(500)
    roomOwner = nil
...
RegisterNetEvent('gvz-gangmotels:roomOptions')
AddEventHandler('gvz-gangmotels:roomOptions', function(room, motel)
    local motelName = nil
    local motelRoom = nil
    for k,v in pairs(Config.Zones) do
        for kk,vm in pairs(v.Rooms) do       
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gvz-gangmotels',
        {
            title    = motelName..' Oda '..motelRoom,
            align    = 'top-right',
            elements = { 
                { label = 'Odaya gir', value = 'enter' },
                { label = 'Kiralamayı iptal et', value = 'cancel' }
            }
        },
    function(data, entry)
        ...
        if value == 'enter' then
            entry.close()
            TriggerEvent("gvz-gangmotels:enterRoom", room, motel)

        elseif value == 'cancel' then
            entry.close()
            TriggerEvent("gvz-gangmotels:cancelRental", room)
        end
...
RegisterNetEvent('gvz-gangmotels:roomMenu')
AddEventHandler('gvz-gangmotels:roomMenu', function(room, motel)
    local motelName = nil
    local motelRoom = nil
    local roomID = nil
    ...
    for k,v in pairs(Config.Zones) do
        ...
        table.insert(options, {label = 'Karakter değiştir', value = 'changechar'})
        end
     --   table.insert(options, {label = 'Odadan çık', value = 'leaveroom'})
        if roomOwner == playerIdent then
        table.insert(options, {label = 'Komidini aç', value = 'inventory'})
    ...
    function(data, menu)
        ...
        menu.close()
        TriggerEvent('gvz-gangmotels:exitRoom', curMotel, curRoom)
        ...
        slots = 80,
            ...
            exports['mythic_notify']:SendAlert('error', 'Buraya sadece oda sahibi erişebilir!')
        end
            
				function OpenPropertyInventoryMenu(motel, owner)
    ESX.TriggerServerCallback(
        "gvz-gangmotels:getPropertyInventory",
        function(inventory)
...
RegisterNetEvent('gvz-gangmotels:rentRoom')
AddEventHandler('gvz-gangmotels:rentRoom', function(room)
    ...
    TriggerServerEvent('gvz-gangmotels:rentRoom', room)
    TriggerEvent("mythic_progressbar:client:progress", {
        name = "renting_motel",
        duration = 2000,
...
function roomMarkers()
	...
	for k,v in pairs(Config.Zones) do
        ...
        if distance < 1.5 then
            DrawText3D(v.roomExit.x, v.roomExit.y, v.roomExit.z + 0.15, ' ~b~[E]~w~ ile odadan çık!')
            DrawMarker(20, v.roomExit.x, v.roomExit.y, v.roomExit.z + -0.11, 0, 0.50, 0, 0, 0, 0, 0.45,0.25,0.25, 255, 0, 0, 255, 0.0, 0.10, 0, 0.0, 0, 0.0, 0)
                if IsControlJustReleased(0, Keys['E']) then
                    TriggerEvent('gvz-gangmotels:exitRoom', curRoom, curMotel)
                end
        end
    end
    ...
    for k,v in pairs(Config.Zones) do
        ...
        if distance < 1.0 then
            DrawText3D(v.Menu.x, v.Menu.y, v.Menu.z + 0.35, 'Menüye erismek için [~b~E~c~] tusuna basınız.')
                if IsControlJustReleased(0, Keys['E']) then
                    TriggerEvent('gvz-gangmotels:roomMenu', curRoom, curMotel)
                end
        end
    end
    ...
    for k,v in pairs(Config.Zones) do
        ...
        if distance < 1.0 then
            if roomOwner == playerIdent then
            DrawText3D(v.Inventory.x, v.Inventory.y, v.Inventory.z + 0.35, 'Kıyafetinizi degistirmek için [~g~E~s~]  tusuna basınız.')
                if IsControlJustReleased(0, Keys['E']) then
                    PlayerDressings()
                end
            end
    ...
    for k,v in pairs(Config.Zones) do
        ...
        if distance < 1.0 then
            if roomOwner == playerIdent then
            DrawText3D(v.BedStash.x, v.BedStash.y, v.BedStash.z + 0.1, 'Esya saklamak icin [~g~E~s~]  tusuna basınız.')
                if IsControlJustReleased(0, Keys['E']) then
                    OpenStash()
                end
            end
    ...
    for k,v in pairs(Config.Zones) do
        for km,vm in pairs(v.Rooms) do
            if vm.instancename == myMotel then
                ...
                if (distance < 1.0) then
                    DrawText3D(vm.entry.x, vm.entry.y, vm.entry.z + 0.35, 'Menüye erismek için [~g~E~s~] tusuna basınız')
                    if IsControlJustReleased(0, Keys['E']) then
                        TriggerEvent("gvz-gangmotels:roomOptions", vm.instancename, k)
                    end
                end
            end
        end
        ...
        for k,v in pairs(Config.Zones) do
            ...
            if (distance < v.Boundries) then
                for km,vm in pairs(v.Rooms) do
                    distance = GetDistanceBetweenCoords(coords, vm.entry.x, vm.entry.y, vm.entry.z, true)
                    if (distance < 1.0) then
                        DrawText3D(vm.entry.x, vm.entry.y, vm.entry.z + 0.35, vm.number..'Numaralı odayı '..Config.PriceRental ..'~b~ TL ye kiralamak için [~g~E~s~] tusuna basınız')
                        if IsControlJustReleased(0, Keys['E']) then
                            TriggerEvent('gvz-gangmotels:rentRoom', vm.instancename)
                        end
                    end
                end
            end
...
function OpenPropertyInventoryMenuBed(property, owner)
	ESX.TriggerServerCallback(
		"gvz-gangmotels:getPropertyInventoryBed",
		function(inventory)
...
function OpenStash()
    ...
    owner = ESX.GetPlayerData().identifier
    ESX.TriggerServerCallback('gvz-gangmotels:checkIsOwner', function(isOwner)
        if isOwner then
                    TriggerEvent("mythic_progressbar:client:progress", {
                        name = "renting_motel",
                        duration = 1500,
                        label = "Yastık altınız açılıyor.",
                        useWhileDead = false,
                        canCancel = true,
                        controlDisables = {
                                disableMovement = true,
